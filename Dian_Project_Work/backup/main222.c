...
int video_play_fps = 8;        // 视频播放帧率（1s内）
...
int cur_collect_thrd = 0;
int handle_frame_index = 0;
...
int main(int argc,char *argv[]){        
    ...
    for (int i = 10;i <= 70;i ++){
        ...
        handle_frame_index = get_frame_index();
        thrd_create(&threads[i],frame_buffer_collect,&f);    
...
*/
    while (true){
        ...
        //printf("%d\t%d\n",current_time,clock());
        if (current_time >= 30 second) {        // 时间限制
            printf("Time over limit.\n");
            break;
        }
        if ( buffer_save[buffer_cur_print - 1].index >= 3000) {        // 帧限制
...
void command_stack_handle(){            // 处理指令栈中的所有参数
    ...
    if (command_stack.s[0][0] != '-') printf("Invalid command was received.\n");    // 参数不合法
        ...
        if (strcmp(command_stack.s[0],"-h") == 0 || strcmp(command_stack.s[0],"--help") == 0){
            ...
            if (command_stack.len == 1) pooling = MAX_POOLING;
            else if (command_stack.len >= 2 && strcmp(command_stack.s[1],"g") == 0) pooling = GRAY_HANDLE;
...
int frame_buffer_collect(void *ptr){
    if ((buffer_cur_collect[cur_collect_thrd] + 10) % BUFFER_MAX == buffer_cur_print) return thrd_error;
    while (buffer_save[buffer_cur_collect[cur_collect_thrd]].is_done != false){
        if ((buffer_cur_collect[cur_collect_thrd] + 10) % BUFFER_MAX == buffer_cur_print) return thrd_error;
        buffer_cur_collect[cur_collect_thrd] = (buffer_cur_collect[cur_collect_thrd] + 1) % BUFFER_MAX;
    }
    Frame *f_ptr = (Frame *) ptr;
    uint8 *buffer_new = (uint8 *)malloc(sizeof(uint8) * new_frame_height * new_frame_width * 3 + 100);
    color color2;

    buffer_save[buffer_cur_collect[cur_collect_thrd]].index = handle_frame_index;
    for (int i = 0;i < new_frame_height;i ++){
        for (int j = 0;j < new_frame_width;j ++){
            if (pooling == MAX_POOLING)
                color2 = max_pooling(f_ptr,pixel_width,pixel_height,i,j);
            else if (pooling == AVERAGE_POOLING)
                color2 = average_pooling(f_ptr,pixel_width,pixel_height,i,j);
    ...
    buffer_save[buffer_cur_collect[cur_collect_thrd]].buffer = buffer_new;
    buffer_save[buffer_cur_collect[cur_collect_thrd]].is_done = true;
    //printf("\tbuffer_save [%d] has been bulit successfully.\n",buffer_cur_collect[cur_collect_thrd]);
    buffer_cur_collect[cur_collect_thrd] = (buffer_cur_collect[cur_collect_thrd] + 1) % BUFFER_MAX;
