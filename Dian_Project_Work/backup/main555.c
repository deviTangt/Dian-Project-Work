...
int is_quit = false;        // 是否退出程序（来自终端输入决定）
int last_print_index = 0;   // 上一次打印的帧序列
clock_t last_same_print_time = (int)1e9;    // 上一次相同帧的时间
...
gcc src/main.c -I include/ -lpthread -L lib/ -lvideodecoder -lavformat -lavcodec -lavutil -lswscale -o main.out
./main.out -h -v 599 --file "dragon.mp4" -r 5 10 -c g p
g
#endif
    ...
    while (true){
        ...
        if (current_time > last_time + (int)(1.0 second / video_play_fps)){  // 控制帧率
            ...
            if (is_play_paused == false) { // 打印帧缓冲
                ...
                printf("time: %d  (%.2fs)\t",(int)current_time,1.0 * current_time / (1 second));
                printf("Video play speed: %.2f\tSource fps equal:%.2f\n",video_play_fps,video_play_fps * (1 + skip_frame));
                //printf("  last_same_time:%d last_index:%d\n",last_same_print_time,last_print_index);
            }


            if (buffer_cur_print == last_print_index && last_same_print_time == (int)1e9)   // 记录相同帧时间
                last_same_print_time = current_time;
            else if (buffer_cur_print != last_print_index){    // 防止卡顿，帧率过快时打印帧序列一般不能到遍历到总帧数的末尾 
                last_print_index = buffer_cur_print;
                last_same_print_time = (int)1e9;
            }
        }
        ...
        else if (current_time - last_same_print_time > 1 second         // 帧相同限制
                    && buffer_save[buffer_cur_print - 1].index >= (int)(0.95 * get_total_frames())){
            printf("The video has been completely played.\n");
            break;
        }
        else if (current_time >= 300 second) {        // 时间限制
            printf("Time over limit.\n");
            break;
        }
        else if ( buffer_save[buffer_cur_print - 1].index >= 30000) {        // 帧限制
            printf("Frame over limit.\n");
            break;
        }
        else if (is_quit){      // 终端按q键退出
            printf("Quit\n");
            break;
        }
#endif
...
void command_stack_handle(){            // 处理指令栈中的所有参数
    ...
    if (command_stack.s[0][0] != '-') printf("Invalid command was received.\n");    // 参数不合法
        ...
        if (strcmp(command_stack.s[0],"-h") == 0 || strcmp(command_stack.s[0],"--help") == 0){
            ...
            if (command_stack.len >= 2 && strcmp(command_stack.s[1],"g") == 0) is_gray = true;
...
int frame_buffer_collect(void *ptr){        // 帧缓存收集
    ...
    char *new_buffer = (char *)malloc(25 * new_frame_height * new_frame_width * 3); // 13 + 9 + 2
                // 帧存储开辟空间，由于一个"\x1b[38;2;%d;%d;%dm█"元素存储16~22字节（依据3个%d数字的位数增加而增加），所以统一设字节量为25
    ...
    for (int i = 0;i < new_frame_height;i ++){      // 保存缓存信息
        for (int j = 0;j < new_frame_width;j ++){
            ...
            new_punct_buffer[punct_pos ++] = gray_to_punct(gray_cal(&color2));
    ...
    buffer_save[buffer_cur_collect[cur_data.cur_thrd_index]].punct_buffer = new_punct_buffer;
    buffer_save[buffer_cur_collect[cur_data.cur_thrd_index]].is_done = true;
/*
gcc src/main.c -I include/ -lpthread -L lib/ -lvideodecoder -lavformat -lavcodec -lavutil -lswscale -o main.out
./main.out -h -v 599 --file "dragon.mp4" -r 5 10 -c g 
*/
...
int frame_buffer_print(void *ptr){              // 帧缓存打印
    ...
    if (is_punct) puts(buffer_save[buffer_cur_print].punct_buffer); // 打印字符图
    else puts(buffer_save[buffer_cur_print].buffer);                // 打印彩图/灰度图

    free(buffer_save[buffer_cur_print].punct_buffer);     // 释放缓存区
...
void key_state_handle(int key){         // 对用户输入进行处理
    switch (key){
        ...
        case 'H': {
            ...
            printf("'q' or 'Q': quit to stop proceedings.\n");
        }  break;
        ...
        case 'g':       // 原彩/灰度图打印与字符图打印模式切换
        case 'G': {
            is_punct ^= true;
            if (is_punct) printf("Play mode has been shifted into punct.\n");
            else if (is_gray)  printf("Play mode has been shifted into gray.\n");
            else printf("Play mode has been shifted into colored.\n");
        }  break;
...
int frame_buffer_print(void *ptr){              // 帧缓存打印
    ...
    free(buffer_save[buffer_cur_print].buf);
    buffer_save[buffer_cur_print].is_done = false;  // 更新缓存去状态
    buffer_cur_print = (buffer_cur_print + 1) % BUFFER_MAX;     // 更新缓存区打印索引
